...
На `in-qa` с открытым PR: `-in-qa +qa-in-progress` → `role: tester`. Тестировщик возвращает `PIPELINE_BUG_ISSUES` (блокеры, ≤2); оркестратор ставит перечисленным issues `bug` + `needs-plan` и возвращает родителя в `in-qa`. Без дефектов: `-qa-in-progress +qa-passed`. Нет маркера, ошибка маркировки или ошибка агента: `+needs-human`. Нет PR `Fixes #N`: `+needs-human` (не skip). Дочерний issue (`Related to #`) не плодит внуков; >2 bugs за прогон → `needs-human`.
