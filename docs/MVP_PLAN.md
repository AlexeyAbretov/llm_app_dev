...
> **Заморожен в релизе 1.0 (2026-09-09).** Исторический документ этапов 0–11. **Не изменять** содержимое, чеклисты и этапы. Дальнейшая работа — GitHub issues и [CONSTITUTION.md](./CONSTITUTION.md), не этот файл.
...
**Статус релиза 1.0:** этапы 0–11 влиты в `main`. Чеклисты «Проверка» с `[ ]` — локальный E2E оператора (Ollama + demo), не пробелы реализации. Этот файл больше не дополняется.

MVP считается завершённым когда:

1. Все 10 этапов пройдены (плюс этап 11 GridFS)
