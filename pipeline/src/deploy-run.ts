...
export async function runCatalogDeploy(config: Config, tag: string): Promise<DeployRunResult> {
  if (config.DEPLOY_MODE === "stub") {
    return {
      ok: true,
      detail: `stub: tag \`${tag}\` доехал, docker compose каталога не запускался (DEPLOY_MODE=stub).`,
    };
  }
